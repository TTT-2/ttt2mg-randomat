L = LANG.GetLanguageTableReference("de")

L["ttt2_weapon_randomat_name"] = "Randomat MG"
L["ttt2_weapon_randomat_desc"] = "Nutze diese Waffe um ein zufälliges Minispiel für diese Runde zu starten."
L["ttt2_weapon_randomat_help_msb1"] = "Führt ein zufälliges Minispiel aus"

--EVENT STRINGS
L["title_mg_randomat"] = "Ein Randomat wurde aktiviert"
L["desc_mg_randomat"] = "{name} hat den Randomat aktiviert."
